  ...
  switch (EMBEDDING_DIMENSION) {
    case OPENAI_EMBEDDING_DIMENSION:
      if (!process.env.OPENAI_API_KEY) {
        throw new Error(
          "¿Estás intentando usar OpenAI? Si es así, ejecuta: npx convex env set OPENAI_API_KEY 'tu-clave'",
        );
      }
      break;
    case TOGETHER_EMBEDDING_DIMENSION:
      if (!process.env.TOGETHER_API_KEY) {
        throw new Error(
          "¿Estás intentando usar Together.ai? Si es así, ejecuta: npx convex env set TOGETHER_API_KEY 'tu-clave'",
        );
      }
      break;
    ...
    default:
      if (!process.env.LLM_API_URL) {
        throw new Error(
          "¿Estás intentando usar un LLM personalizado en la nube? Si es así, ejecuta: npx convex env set LLM_API_URL 'tu-url'",
        );
      }
      break;
  }
...
export function getLLMConfig(): LLMConfig {
  ...
  if (provider ? provider === 'openai' : process.env.OPENAI_API_KEY) {
    if (EMBEDDING_DIMENSION !== OPENAI_EMBEDDING_DIMENSION) {
      throw new Error('EMBEDDING_DIMENSION debe ser 1536 para OpenAI');
    }
    return {
      provider: 'openai',
  ...
  if (process.env.TOGETHER_API_KEY) {
    if (EMBEDDING_DIMENSION !== TOGETHER_EMBEDDING_DIMENSION) {
      throw new Error('EMBEDDING_DIMENSION debe ser 768 para Together.ai');
    }
    return {
      provider: 'together',
  ...
  if (process.env.LLM_API_URL) {
    ...
    if (!chatModel) throw new Error('LLM_MODEL es requerido');
    const embeddingModel = process.env.LLM_EMBEDDING_MODEL;
    if (!embeddingModel) throw new Error('LLM_EMBEDDING_MODEL es requerido');
    return {
      provider: 'custom',
      url,
      chatModel,
