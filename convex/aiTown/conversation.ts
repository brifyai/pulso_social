...
export class Conversation {
  ...
  static start(game: Game, now: number, player: Player, invitee: Player) {
    if (player.id === invitee.id) {
      throw new Error(`No puedes invitarte a ti mismo a una conversación`);
    }
    // Ensure the players still exist.
    if ([...game.world.conversations.values()].find((c) => c.participants.has(player.id))) {
      const reason = `El jugador ${player.id} ya está en una conversación`;
      console.log(reason);
      return { error: reason };
    }
  ...
  setIsTyping(now: number, player: Player, messageUuid: string) {
    if (this.isTyping) {
      if (this.isTyping.playerId !== player.id) {
        throw new Error(`El jugador ${this.isTyping.playerId} ya está escribiendo en ${this.id}`);
      }
      return;
    }
  ...
  acceptInvite(game: Game, player: Player) {
    ...
    if (!member) {
      throw new Error(`El jugador ${player.id} no está en la conversación ${this.id}`);
    }
    if (member.status.kind !== 'invited') {
      throw new Error(
        `Estado de membresía inválido para ${player.id}:${this.id}: ${JSON.stringify(member)}`,
      );
    }
  ...
  rejectInvite(game: Game, now: number, player: Player) {
    ...
    if (member.status.kind !== 'invited') {
      throw new Error(
        `Rechazando invitación en estado de membresía incorrecto: ${this.id}:${player.id}: ${JSON.stringify(
          member,
        )}`,
      );
    }
  ...
  leave(game: Game, now: number, player: Player) {
    const member = this.participants.get(player.id);
    if (!member) {
      throw new Error(`No se pudo encontrar la membresía para ${this.id}:${player.id}`);
    }
    this.stop(game, now);
  }
