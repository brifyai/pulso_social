  ...
  static join(
    ...
    if (tokenIdentifier) {
      ...
      if (numHumans >= MAX_HUMAN_PLAYERS) {
        throw new Error(`Solo se permiten ${MAX_HUMAN_PLAYERS} jugadores humanos a la vez.`);
      }
    }
    let position;
    ...
    if (!position) {
      throw new Error(`¡No se pudo encontrar una posición libre!`);
    }
    const facingOptions = [
      { dx: 1, dy: 0 },
      { dx: -1, dy: 0 },
    ...
    if (!characters.find((c) => c.name === character)) {
      throw new Error(`Personaje inválido: ${character}`);
    }
