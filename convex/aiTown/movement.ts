...
export function movePlayer(
  ...
  if (Math.floor(destination.x) !== destination.x || Math.floor(destination.y) !== destination.y) {
    throw new Error(`Destino no integral: ${JSON.stringify(destination)}`);
  }
  ...
  if (inConversation && !allowInConversation) {
    throw new Error(`No puedes moverte cuando estás en una conversación. ¡Sal de la conversación primero!`);
  }
  player.pathfinding = {
